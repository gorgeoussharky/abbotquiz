...
import doctor from '../../../../../assets/img/kidney.png';
...
const Remission = ({ onBack }: Props) => {
  const dispatch = useAppDispatch();

  const recommendations = [
    {
      title: 'Отмена спазмолитиков повышает риск рецидива симптомов',
      icon: drugs,
    },
    {
      title:
        'При долгосрочном наблюдении пациента с СРК возможна смена фенотипа функциональных заболеваний ЖКТ, поэтому важно наблюдать за симптомами и корректировать терапию согласно текущим проявлениям',
      icon: schedule,
    },
    {
      title:
        'Рационально рекомендовать пациенту ежегодное посещение гастроэнтеролога с целью контроля состояния (в случае отсутствия более раннего рецидива)',
      icon: doctor,
    },
    ...
    <QuizWrap>
      <ColumnsWrap>
        <Column>
          ...
          <DiagnosisCard>
            СРК — хроническое заболевание и ремиссия может поддерживаться только
            при поддержании терапии, поэтому рекомендовано продолжать базисную
            терапию в зависимости от тяжести СРК.
          </DiagnosisCard>
